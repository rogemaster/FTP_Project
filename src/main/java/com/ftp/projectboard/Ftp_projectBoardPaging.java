...
public class Ftp_projectBoardPaging {

	private int pageSize = 5;		// 페이지당 게시글 수
	private int rangeSize = 5;		// 블록당 페이지 수
	...
	public Ftp_projectBoardPaging(int listCnt, int curPage) {
		// TODO Auto-generated constructor stub
		
		setCurPage(curPage);		// 현재 페이지
		setListCnt(listCnt);		// 총 게시물 수
		setPageCnt(listCnt);		// 총 페이지 수
		setRangeCnt(pageCnt);		// 총 블록(rang) 수
		rangeSetting(curPage);		// 블록 (rang) 설정(setting)
		setStartIndex(curPage);		// DB에 의한 startIndex 설정
